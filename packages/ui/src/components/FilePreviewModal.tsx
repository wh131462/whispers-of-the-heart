...
export const FilePreviewModal: React.FC<FilePreviewModalProps> = ({
  ...
  return (
    <div className={cn("fixed inset-0 z-50 bg-black", className)}>
      ...
      <button
        onClick={onClose}
        className="absolute top-6 right-6 z-30 w-12 h-12 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
      >
      ...
      {showNavigation && canGoToPrevious && (
        <button
          ...
          className="absolute left-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showNavigation && canGoToNext && (
        <button
          onClick={() => onFileChange?.(currentIndex + 1)}
          className="absolute right-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-all duration-200 hover:scale-110"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {hasMultipleFiles && showNavigation && (
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 px-4 py-2 bg-black/50 backdrop-blur-sm rounded-full text-white text-sm font-medium">
