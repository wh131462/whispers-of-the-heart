...
> 专为AI高效执行项目任务设计的文档系统(与 `openspec/` 规格体系协同)

## 📚 文档索引

| 文档                                           | 用途           | 何时使用           | 更新频率     |
| ---------------------------------------------- | -------------- | ------------------ | ------------ |
| [0-INDEX.md](./0-INDEX.md)                     | 体系导航(必读) | 不确定读哪个文档时 | 体系变化时   |
...
- **`.ai/`** = 工作背景层(描述"现在是什么样")
- **`openspec/`** = 规格演进层(驱动"接下来要变成什么样")

详见 [0-INDEX.md](./0-INDEX.md) 的决策树,了解 AI 何时该读哪里。

## 🚀 快速开始

### 助手入口

- Claude Code 自动读取仓库根目录的 `CLAUDE.md`。
- Codex 自动读取仓库根目录的 `AGENTS.md`。
- 两个入口均按需加载本目录与 `openspec/`，项目知识只在此处维护一份。
...
2. **日常开发**:
   - 未支持项目入口文件的 AI 需要在任务中引用相关文档；Claude Code 和 Codex 无需重复提示
...
第六层: openspec/specs/<cap>/   (涉及该领域时)
第七层: openspec/changes/<id>/  (跟进具体变更时)
```
...
**创建日期**: 2025-12-25
**最后更新**: 2026-07-22
**适用于**: Claude Code、Codex 和其他 AI 编程助手
