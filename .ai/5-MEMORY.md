...
### 2026-09-23 - 支持查看作品完整介绍

**问题**：首页精选和紧凑卡片的详细介绍被行数限制截断，用户没有明确入口查看全文。

**实施**：作品列表标准卡片默认展示完整详细介绍；首页主卡和紧凑卡保留预览，并在详细介绍超过 180 个字符时提供“展开完整介绍/收起介绍”按钮。

**验证**：Web 类型检查、作品卡片 ESLint/Prettier 检查和 `git diff --check` 通过。

### 2026-09-23 - 优化作品卡片默认封面

**问题**：没有上传封面或封面加载失败时，卡片只显示单一渐变和居中图标，视觉重量不足。

**实施**：将默认封面改为抽象作品集背景：柔和光晕、低对比斜纹、项目名称首字母水印和半透明图标牌；真实封面继续使用原有图片与遮罩，默认封面保留悬停动效并兼容暗色模式。

**验证**：Web 类型检查、作品卡片 ESLint/Prettier 检查和 `git diff --check` 通过。

### 2026-09-23 - 融合作品卡片的介绍层级

**问题**：上一版详细介绍使用标签和左侧边框，信息块感过强，与作品集卡片的视觉不融合。

**实施**：移除“详细介绍”标签和边框，将摘要与详细介绍合并为连续叙述；摘要提升为正文主层级，详细介绍使用更轻的字号、颜色和首行微标记，并按卡片变体保留行数限制。

**验证**：Web 类型检查、作品卡片 ESLint/Prettier 检查和 `git diff --check` 通过。

### 2026-09-23 - 优化首页与作品列表的介绍信息布局

**需求**：让不同长度、缺少详细介绍的作品在首页精选和作品列表中都保持可读、稳定的卡片布局。

**实施**：共享作品卡片按摘要与详细介绍分层；详细介绍增加左侧强调线和标签，支持换行/断词；首页主卡、次卡及列表卡分别限制摘要和详细介绍行数，避免长文本撑高卡片并保持操作区对齐。

**验证**：Web 类型检查、作品相关文件 ESLint/Prettier 检查和 `git diff --check` 通过；保留 HomePage 原有 console warning。

### 2026-09-23 - 在作品卡片展示详细介绍

**问题**：后台可保存作品的详细介绍，但首页和 `/projects` 作品卡片只渲染一句话介绍，导致详细介绍完全不可见。

**实施**：复用现有 `description` 字段，在共享 `ProjectShowcaseCard` 中按“摘要 + 详细介绍”层级展示；标准卡片保留完整文本，首页精选卡片按变体限制行数，支持换行和长文本断词。

**验证**：Web 类型检查、作品相关文件 ESLint/Prettier 检查和 `git diff --check` 通过；保留 HomePage 原有 console warning。

### 2026-09-23 - 移除弹窗内容区的二层边距容器

**问题**：媒体选择和作品编辑弹窗的滚动区外又包了一层带内边距、圆角和边框的内容卡片，视觉上像嵌套弹窗。

**实施**：移除这层二级容器，让中间区域直接作为唯一滚动区；保留列表/表单自身的内容留白、固定头部和固定底部操作区。

**验证**：Web 类型检查、定向 ESLint、Prettier、`git diff --check` 和生产构建通过；仅保留既有构建 warning。
