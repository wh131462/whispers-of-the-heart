...
**任务概览**：对约 487 个 TS/TSX 文件、8.6 万行代码执行静态逻辑审查，并集中修复报告中的 P0/P1/P2 逻辑问题。

**关键发现**：公开文章接口可读取草稿；媒体更新缺少所有权校验；评论可跨文章回复；密码重置缺少事务和会话撤销；AI 配额可越界且仅存内存；异常过滤器丢失媒体引用详情。完整报告见 `reports/CODE_LOGIC_AUDIT_2026-07-21.md`。

**实施**：修复草稿泄露、媒体越权、跨文章回复；认证加入事务轮换与 `tokenVersion` 即时撤销；验证码加入安全随机、冷却和失败锁定；AI 配额迁移到数据库原子预留；异常响应保留详情；文章组合写入改为事务。新增迁移 `20260721000000_add_user_token_version`，同时创建 `ai_usage_windows`。

**验证**：`pnpm lint`、`pnpm type-check`、`pnpm test`、`pnpm build` 全部通过；4 个测试套件、6 个测试通过。Lint 仍有历史 warning，前端大 chunk 警告仍待性能专项处理。

**后续**：部署前执行 Prisma migration；逐步清理历史类型安全、Hook 依赖和前端大 chunk 警告。
