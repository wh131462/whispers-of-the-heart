...
界面 MUST 区分入房和传输可用，并提供直连或服务器中转的连接信息。聊天和文件传输界面 MUST 仅在连接状态的悬停提示中展示连接方式，不常驻显示中转标签或说明；多人房间混用通道时 MUST 明确说明。帮助 MUST 说明中转内容经过业务服务器且没有应用层端到端加密。

#### Scenario: Relay active

- **WHEN** 使用服务器中转
- **THEN** 聊天的“已连接”和文件传输的在线状态悬停提示显示服务器中转，不能声称内容不经过服务器
